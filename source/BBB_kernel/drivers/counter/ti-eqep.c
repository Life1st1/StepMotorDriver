...
static struct counter_device *pub_counter[MAX_MOTORS] = { NULL };

struct counter_device** ti_eqep_get_counter(void)
{
	return pub_counter;
}
EXPORT_SYMBOL(ti_eqep_get_counter);
...
static int ti_eqep_probe(struct platform_device *pdev)
{
	...
	if (motor_count >= MAX_MOTORS) {
		dev_err(&pdev->dev, "Too many motors probed\n");
		return -EINVAL;
	}
	pub_counter[motor_count] = &priv->counter;
	motor_count++;
